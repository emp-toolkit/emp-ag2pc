...
static_assert(SessionIO<AG2PCSession, AG2PCSession::Int<32>>);
static_assert(SessionIO<AG2PCSession, AG2PCSession::BitVec<128>>);
static_assert(CheckpointingSession<AG2PCSession>);

// Ctx is the pure gate recorder; the value layer is built over it.
static_assert(std::is_same_v<AG2PCSession::Ctx, AG2PCCtx>);
static_assert(BooleanContext<AG2PCCtx>);
static_assert(std::is_same_v<AG2PCSession::UInt<32>, UInt_T<AG2PCCtx, 32>>);

// AG2PC reveal is recipient-only: reveal_t<V> is std::optional<V::clear_t>.
static_assert(std::is_same_v<
    AG2PCSession::reveal_t<AG2PCSession::UInt<32>>,
    std::optional<AG2PCSession::UInt<32>::clear_t>>);
